...
class Bill_App:
    def __init__(self,root):
        ...
        #============================Total calculatio n in txt area
        self.bill_calc=StringVar()
        ...
        c1_lbl=Label(F4,text="Mazza:",font=("times new roman",16,"bold"),bg=bg_color,fg="lightgreen").grid(row=0,column=0,padx=10,pady=10,sticky="w")
        c1_txt=Entry(F4,width=10,textvariable=self.mazza,font=("times new roman",16,"bold"),bd=5,relief=SUNKEN).grid(row=0,column=1,padx=10,pady=10)
        ...
        F5.place(x=1005,y=180,width=275,height=380)
        ...
        m1_txt=Entry(F6,width=18,textvariable=self.cosmetic_price,font="arial 10 bold",bd=7,relief=SUNKEN).grid(row=0,column=1,pady=1)
        ...
        m2_txt=Entry(F6,width=18,textvariable=self.grocery_price,font="arial 10 bold",bd=7,relief=SUNKEN).grid(row=1,column=1,pady=1)
        ...
        m3_txt=Entry(F6,width=18,textvariable=self.cold_drink_price,font="arial 10 bold",bd=7,relief=SUNKEN).grid(row=2,column=1,pady=1)
        ...
        c1_txt=Entry(F6,width=18,textvariable=self.cosmetic_tax,font="arial 10 bold",bd=7,relief=SUNKEN).grid(row=0,column=3,pady=1)
        ...
        c2_txt=Entry(F6,width=18,textvariable=self.grocery_tax,font="arial 10 bold",bd=7,relief=SUNKEN).grid(row=1,column=3,pady=1)
        ...
        c3_txt=Entry(F6,width=18,textvariable=self.cold_drink_tax,font="arial 10 bold",bd=7,relief=SUNKEN).grid(row=2,column=3,pady=1)

        btn_F=Frame(F6,bd=7,relief=GROOVE)
        btn_F.place(x=720,width=530,height=105)

        total_btn=Button(btn_F,command=self.total,text="Total",bg="cadetblue",fg="white",bd=2,pady=15,width=11,font="arial 12 bold").grid(row=0,column=0,padx=5,pady=10)  
        Bll_btn=Button(btn_F,text="Generate Bill",command=self.bill_area,bg="cadetblue",fg="white",bd=2,pady=15,width=11,font="arial 12 bold").grid(row=0,column=1,padx=5,pady=10)
        Clear_btn=Button(btn_F,text="Clear",command=self.clear_data,bg="cadetblue",fg="white",bd=2,pady=15,width=11,font="arial 12 bold").grid(row=0,column=2,padx=5,pady=10)
        Exit_btn=Button(btn_F,text="Exit",command=root.destroy,bg="cadetblue",fg="white",bd=2,pady=15,width=11,font="arial 12 bold").grid(row=0,column=3,padx=5,pady=10)
    ...
    def total(self):
        ...
        #=============================syntax of round
        #round((value),2)
        #2 denotes till how many decimal points do we want to calculate?display the value.
        ...
        self.grocery_tax.set("Rs. "+str(round((self.total_grocery_price*0.10),2)))     

        self.cd_m_p=self.mazza.get()*40
        self.cd_c_p=self.coke.get()*45
        self.cd_f_p=self.frooti.get()*40
        self.cd_tu_p=self.thumbs_up.get()*35
        self.cd_l_p=self.limca.get()*50
        self.cd_s_p=self.sprite.get()*45
        ...
        self.c_p=self.total_cosmetic_price
        self.g_p=self.total_grocery_price
        self.c_d_p=self.total_cold_drink_price
        self.total_bill=float(
                              self.c_p+
                              self.g_p+
                              self.c_d_p
                             )
        self.bill_calc.set("Rs. "+str(self.total_bill))                     

    def welcome_bill(self):   
        self.txtarea.delete('1.0',END)
        self.txtarea.insert(END,"\t  Welcome Abhi\n")
    ...
    def bill_area(self):
        if self.c_name.get()=="" or self.c_phone.get()=="":
            messagebox.showerror("Error","Enter Costomer Details")
        elif self.cosmetic_price.get()=="Rs. 0.0" and self.grocery_price.get()=="Rs. 0.0" and self.cold_drink_price.get()=="Rs. 0.0":
            messagebox.showerror("Error","No Product Selected")     
        else:
            self.welcome_bill()
            #===============================Condition For Cosmetic Frame
            if self.soap.get()!=0:
            ...
            #===============================Condition For Cold Drink Frame
            if self.mazza.get()!=0:
            ...
            #===============================Total Price Calculation    
            self.txtarea.insert(END,"\n=============================")
            self.txtarea.insert(END,f"\n  Total Bill\t      {self.bill_calc.get()}")
            self.txtarea.insert(END,"\n=============================")
            self.save_bill()    

    def save_bill(self):
        op=messagebox.askyesno("Save bill","Do you yant to save the Bill?")
        if op>0:
            if not os.path.exists('D:\\bills\\'):
                os.makedirs('D:\\bills\\') 
